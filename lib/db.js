...
export async function getEventById(idOrSlug) {
  if (eventsCache && eventsCache.length > 0) {
    const found = eventsCache.find(e => e.slug === idOrSlug || String(e.id) === String(idOrSlug));
...
export async function getInitiativeById(idOrSlug) {
  if (initiativesCache && initiativesCache.length > 0) {
    const found = initiativesCache.find(i => i.slug === idOrSlug || String(i.id) === String(idOrSlug));
    if (found) return found;
  }
  await ensureTablesExist();
  const currentPool = getPool();
